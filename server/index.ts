...
const ACCESS_PASSWORD_FILE = path.join(rootDir, '.app-access-password');
...
function readAccessPasswordFile(): string {
  try {
    if (!fs.existsSync(ACCESS_PASSWORD_FILE)) return '';
    const raw = fs.readFileSync(ACCESS_PASSWORD_FILE, 'utf8');
...
function getConfiguredAccessPassword(): string {
  const stored = process.env.APP_ACCESS_PASSWORD || process.env.ACCESS_PASSWORD || readAccessPasswordFile() || DEFAULT_ACCESS_PASSWORD;
